...
async def driver_accept_handler(query: CallbackQuery):
    ...
    Yo'lovchilar soni {order.count}\n
    Jinsi: {user.jinsi}\n
    Yuk: {order.yuk if len(order.yuk) < 12 else "rasmda"}\n
...
<b>#{order_id} {driver.id}</b>
    ...
    if query.data == 'ok':
        ...
        if order.driver_id:
            ...
            text = f"""Siz allaqachon haydovchi biriktirilgan
Bekor qilmoqchi bolsangiz tugmani bosing
...
🔢 <b>Davlat raqami:</b> {driver.car_number}
    ...
    if query.data == 'cancel':
        ...
        if len(message_text_split) == 4:
            order_id = message_text_split[3].split()[0].lstrip('#').strip()
            driver_id = message_text_split[3].split()[1].lstrip('#').strip()
            order = await Order.get(int(order_id))
            name=(await User.get(id_=int(order.passenger_id))).name
            drivers = await Driver.get_all()
            # order = await Order.get(id_=int(order_id))
            image=order.yuk if len(order.yuk)>12 else None
            if order.driver_id:
                await Order.update(int(order_id), driver_id=None)
                await query.message.bot.send_message(chat_id=driver_id,text="❌Mijoz kelishuvni bekor qildi")
                if not image:
                    await send_order_to_drivers(drivers,name, order.id, query.message, order.dropoff_location,
                                            order.order_type, order.sana, order.passenger_id)
                else:
                    await send_order_to_drivers(drivers, name, order.id, query.message, order.dropoff_location,
                                                order.order_type, order.sana, order.passenger_id,image=image)
                ...
                text = """
                               ⚠️<i>Sizning</i> buyurtmangiz <b>kutish</b> holatida
                               """

...
async def name_handler(message: Message, state: FSMContext) -> None:
    ...
    if len(yuk) > 10:
        await send_order_to_drivers(drivers, name, order.id, message, address=manzil, order_type=order_type, date=sana,
                                    user_id=message.from_user.id, image=yuk)
    else:
        await send_order_to_drivers(drivers, name, order.id, message, address=manzil, order_type=order_type, date=sana,
...
async def send_order_to_drivers(drivers, user, order_id, message, address, order_type, date, user_id, image=None):
    tasks = []
...
Batafsil ma'lumot uchun tugmani bosing
    ...
    if image is not None:
        ...
        photo_id = 'AgACAgIAAxkBAAIB82fJd1HpEYYgPo7nmRtzahmrz71HAALr6jEb8O9JSfphoupfG2OoAQADAgADeQADNgQ' \
            if address.split()[0] == 'Andijon' else \
            'AgACAgIAAxkBAAIB8WfJd0JxwCcEqOGR9FRZkqpkfoBEAALs6jEb8O9JSe5Y8mr06A5nAQADAgADeQADNgQ'
